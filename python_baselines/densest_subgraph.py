...
def edge_vertex_ratio(G):
    """
    Returns the ratio of number of edges to number of vertices in the graph.

    Parameters:
    - G (networkx.Graph): The graph.

    Returns:
    - float: The edge-to-vertex ratio (m / n).
    """
    n = G.number_of_nodes()
    m = G.number_of_edges()
    return m / n if n > 0 else 0.0

def find_best_vertex_to_increase_ratio(G, nodes):
    """
    Finds the best vertex (from current node set) to remove to increase the ratio.
    
    Parameters:
    - G: The original graph
    - nodes: Set of active nodes (defining the view)

    Returns:
    - (best_vertex, new_ratio)
    """
    current_subgraph = G.subgraph(nodes)
    original_ratio = edge_vertex_ratio(current_subgraph)
    best_vertex = None
    ...
    for v in nodes:
        ...
        new_ratio = edge_vertex_ratio(subG)
        if new_ratio > best_ratio:
            best_ratio = new_ratio
            best_vertex = v
        new_nodes.add(v)

    return best_vertex, best_ratio

def iteratively_improve_graph(G):
    """
    Iteratively removes vertices via views to maximize edge-to-vertex ratio.

    Returns:
    - best_nodes: Set of nodes in the best subgraph
    - best_ratio: Highest edge-to-vertex ratio achieved
    """
    nodes = set(G.nodes())
    best_nodes = set(nodes)
    best_ratio = edge_vertex_ratio(G.subgraph(nodes))

    while len(nodes) > 1:
        v, new_ratio = find_best_vertex_to_increase_ratio(G, nodes)
        print(len(nodes))
        if v is None:
        ...
        if new_ratio > best_ratio:
            best_ratio = new_ratio
            best_nodes = set(nodes)  # Copy current best set

    return best_nodes, best_ratio
...
if __name__ == '__main__':
    ...
    print(f"Remaining nodes: {best_G.number_of_nodes()}, edges: {best_G.number_of_edges()} elapsed: {elapsed_time}")
